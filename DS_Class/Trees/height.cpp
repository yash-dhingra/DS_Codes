...
public:
    ...
    int height_helper(Node *ptr)
    {
        int x = 0, y = 0;
        if (ptr != NULL)
        {
            x = height_helper(ptr->lp);
            y = height_helper(ptr->rp);
            if (x > y)
                return x + 1;
            else
                return y + 1;
...
int main()
{
    int array[] = {1, -1, 3, 4, 5, -1, 7};
    int len = sizeof(array) / sizeof(array[0]);
    Tree tree;
    tree.insert_list(array, len);

    cout << "Height of Nodes Present in the Tree: " << tree.height() << endl;
}
