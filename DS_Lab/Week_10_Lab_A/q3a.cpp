...
public:
    ...
    void insert_list(char array[], int len)
    {
        head = new Node;
        head->data = array[0];
    ...
    void level_print_helper(Node *ptr, queue qu)
    {
        if (ptr != NULL)
        {
            cout << ptr->data << " ";
            ...
            if (ptr->rp != NULL)
                qu.insert(ptr->rp);
            ptr = qu.pop();
            level_print_helper(ptr, qu);
        }
    }
    void level_print_recursive()
    {
        queue qu;
        level_print_helper(head, qu);
    }
};
...
int main()
{
    ...
    cout << "\nLevel Print Recursive: ";
    tree.level_print_recursive();
}
